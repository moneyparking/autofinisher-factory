...
def _packet_payload(slug: str) -> dict:
    return {
        ...
        'description': 'A buyer-facing spreadsheet download for simple budgeting.',
        'tags': [f'tag-{index}' for index in range(1, 14)],
        'category': 'x',
        'price_anchor': 1,
        'format_hint': 'Google Sheets + PDF preview',
        'artifacts': {
            'deliverable_path': 'x',
            'preview_path': 'x',
            'master_path': 'x',
            ...
            'seo_path': 'x',
            'source_csv_path': 'x',
            'rendered_listing_html_path': 'x',
            'listing_image_plan_path': 'x',
        },
        'source_provenance': {'niche_id': 'x', 'winner_path': '', 'sku_task_path': ''},
        'digital_product_spec_path': 'x',
        'artifact_manifest_path': 'x',
        'listing_title': 'Budget Spreadsheet | Finance Tracker | Instant Download',
        'buyer_promise': 'x',
        'short_description': 'x',
        'long_description': 'x',
        'feature_bullets': ['x'],
        ...
        'description_terms': 'x',
        'description_compatibility': 'x',
        'photo_sequence_hint': 'x',
        ...
        'listing_image_plan': [
            {'slot': index, 'headline': f'h{index}', 'purpose': 'preview', 'detail': 'x', 'asset_hint': 'x'}
            for index in range(1, 11)
        ],
        'listing_image_paths': [f'listing_image_{index:02d}.png' for index in range(1, 11)],
        'market_readiness': {
            'buyer_facing_copy': True,
            'seo_aeo_complete': True,
            'title_readable': True,
            'tags_ready': True,
