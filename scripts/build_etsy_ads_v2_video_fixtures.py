...
FIXTURE_ROOT = BASE_DIR / 'fixtures' / 'regression' / 'etsy_ads_cluster_v2'
...
def build_video_fixture(video_id: str, role: str) -> dict[str, Any]:
    ...
    return {
        'video_id': video_id,
        'role': role,
        'fixture_version': 'etsy_ads_cluster_v2',
        'expected_contract': {
            'quality_gate': {
                'passed': True,
                'wedge_mode': True,
...
def main() -> int:
    ...
    for entry in inputs.get('videos') or []:
        fixture = build_video_fixture(entry['video_id'], entry.get('role', 'batch'))
        fixtures.append(fixture)
        write_json(VIDEO_FIXTURE_DIR / f"{entry['video_id']}.json", fixture)
    report = {
        'fixture_root': str(FIXTURE_ROOT),
        'video_fixture_count': len(fixtures),
        'batch_video_count': len([item for item in fixtures if item['role'] == 'batch']),
        'control_video_count': len([item for item in fixtures if item['role'] == 'control']),
